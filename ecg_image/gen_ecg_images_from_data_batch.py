...
def run(args):
    ...
    if os.path.isabs(args.input_directory) == False:
        args.input_directory = os.path.normpath(
            os.path.join(os.getcwd(), args.input_directory)
        )
    if os.path.isabs(args.output_directory) == False:
        original_output_dir = os.path.normpath(
            os.path.join(os.getcwd(), args.output_directory)
        )
    else:
        original_output_dir = args.output_directory

    if (
        os.path.exists(args.input_directory) == False
        or os.path.isdir(args.input_directory) == False
    ):
        raise Exception(
            "The input directory does not exist, Please re-check the input arguments!"
        )
    ...
    i = 0
    full_header_files, full_recording_files = find_records(
        args.input_directory, original_output_dir
    )
    ...
    for full_header_file, full_recording_file in zip(
        ...
        args.output_directory = os.path.join(
            original_output_dir, "/".join(folder_struct_list)
        )
